...
makeCacheMatrix <- function(x = matrix()) {
  ...
  set <- function(y) {
    ...
    s <<- NULL ## re-initialize s in the parent environment to null
  }
  get <- function() x ## return the matrix x
  setinverse <- function(inverse) s <<- inverse ## set the cache s equal to the inverse of the matrix x
  getinverse <- function() s ## return the cached inverse of x
